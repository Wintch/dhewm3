...
bool Sys_GetPath(sysPath_t type, idStr &path) {
	...
	case PATH_CONFIG:
		...
		if (s)
			idStr::snPrintf(buf, sizeof(buf), "%s/drap", s);
		else
			idStr::snPrintf(buf, sizeof(buf), "%s/.config/drap", getenv("HOME"));
	...
	case PATH_SAVE:
		...
		if (s)
			...
			idStr::snPrintf(buf, sizeof(buf), "%s/.local/share/drap", getenv("HOME"));
